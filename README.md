...
当前版本已提供可由 Codex 使用的 MVP：`Training Core + Trainer MCP + PostgreSQL + Knowledge Trainer Skill`。Java 后端面试知识是首个领域原型，但平台核心不绑定 Java。
...
| Knowledge Trainer Skill | 可用 | 领域无关、无状态、面向 Codex 的训练工作流 |
...
该文件使用仅当前用户可读写的权限，不进入项目仓库；状态查询也不会返回 Token。之后可以直接使用 `$knowledge-trainer` 开始训练。
...
这种方式仍需单独安装或加载 [`skills/knowledge-trainer`](skills/knowledge-trainer)，推荐普通用户优先使用 Plugin。
...
├── skills/
│   └── knowledge-trainer/        # 领域无关、无状态的 AI 教练 Skill
├── contracts/
...
- [Knowledge Trainer Skill](skills/knowledge-trainer/SKILL.md)
