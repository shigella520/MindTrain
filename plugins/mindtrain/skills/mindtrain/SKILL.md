...
1. Call `create_training_session` without `questionCount` unless the user requests a specific count; Core defaults to its configured daily target. Use provider ID `weighted`, displayed to users as `加权调度`.
