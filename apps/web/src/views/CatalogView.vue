...
import { BookOpen, Check, Copy, FileQuestion, FolderTree, Search, Sparkles } from '@lucide/vue'
...
async function copyPrompt() {
  await navigator.clipboard.writeText('使用 $mindtrain，根据我的学习目标创建一个训练领域。请先展示完整知识点树，等我确认后再保存。')
  copied.value = true
...
<template>
  <main class="catalog-page page-container">
    <header class="catalog-heading">
      <div><p class="eyebrow">KNOWLEDGE CATALOG</p><h1>知识目录</h1><p>浏览训练领域、知识点树、题目覆盖与掌握情况。</p></div>
    ...
    <section v-else-if="!loading && !domains.length" class="catalog-empty content-card">
      <Sparkles :size="42" /><h2>还没有训练领域</h2><p>在 Codex 中告诉 MindTrain 想学什么，或指定一个本地资料目录。确认知识点树后才会保存。</p>
    ...
    <section v-else class="catalog-workspace">
      <aside class="catalog-domains">
        <div class="catalog-panel-title"><BookOpen :size="18" /><strong>训练领域</strong><span>{{ domains.length }}</span></div>
      ...
      <section class="catalog-tree-panel">
        <div class="catalog-panel-title"><FolderTree :size="18" /><strong>{{ tree?.domain.name || '知识点树' }}</strong><span>{{ tree?.domain.topicCount || 0 }}</span></div>
        ...
        <ul v-else class="catalog-tree">
          <KnowledgeTreeNode v-for="root in tree?.roots || []" :key="root.id" :node="root" :selected-id="topic?.id" :matched-ids="matchedIds" @select="selectTopic" />
        </ul>
      </section>
