...
首次配置和每次任务开始时还会校验 Plugin 与 Trainer MCP 的组件版本、契约版本和服务端要求的最低 Plugin 版本：

- 组件版本一致：正常使用。
- 组件版本不同但契约兼容：提醒同步升级，但不阻断训练。
- 契约不兼容、Plugin 低于最低版本或服务端不支持版本握手：阻断远程工具调用，并提示升级对应组件。

Trainer MCP 会在每次远程工具调用时再次校验请求头。因此，即使旧 Plugin 已经保存过配置，服务端升级后也不会静默使用不兼容接口。
...
| Plugin/服务端版本不兼容 | 同步升级服务端和 Plugin，重新安装 Plugin 后开启新任务；不要绕过兼容性校验 |
