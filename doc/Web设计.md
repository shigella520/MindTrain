...
| `/admin` | 可复习题/待答 AI 临时题概览、薄弱知识点和实例状态 |
...
1. 顶部学习 Hero：今日完成数与可配置目标、到期积压、新题额度、准确率和开始训练入口。
2. 今日调度：复习题、新题额度、累计作答和会话数量。
3. 学习洞察：薄弱主题热力图和累计正确率。
4. 内容治理：可复习题、待答 AI 临时题和 Scheduler Provider。
...
Core 的报告 API 返回按配置时区统计的“今日已完成主问题数”、每日目标以及复习/新题预算。Dashboard 不计入追问，也不再使用固定的 `0/10` 占位数据。
