...
Core 创建 Assignment；缺题时返回 generationProfile
    ↓
Skill 展示题干和 A-D，或按指定题型、难度和知识点生成候选题
    ↓
用户回答或自由追问
    ↓
Core 解析并精确判分
...
`generationProfile` 由 Core 生成，至少包含 `questionType`、`difficulty` 和 `knowledgePoint`。Core 根据当前 Session 已出题型的数量选择较少的一类，根据知识点掌握度校准难度；主问题候选题提交时必须匹配该 Profile。Skill 负责题面质量与来源核验，不能自行改变 Profile。
