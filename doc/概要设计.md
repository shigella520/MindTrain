...
### 5.3 Knowledge Trainer Skill

Skill 工作名为 `knowledge-trainer`，作为 AI 教练行为层：
...
同源部署由 Web Nginx 将 `/api/*` 转发到容器网络中的 Core。Web 不直接访问 PostgreSQL，也不承担 AI 候选题生成；当 Core 返回 `generation_required` 时，引导用户使用 Codex Knowledge Trainer。
...
当前仓库已经提供 Trainer MCP 和 `knowledge-trainer` Skill。旧 Java Skill 继续保留，直到真实训练数据完成迁移验收。
