...
MindTrain 因此转型为通用知识训练平台。Codex Skill 仍是重要入口，但不再是系统本体；早期 Java 面试内容只作为迁移和验证用的用户数据，不构成平台边界或内置题库。
...
8. **领域由用户定义**：核心模型不绑定 Java；用户通过 AI 对话、本地资料或受控导入创建并确认自己的训练领域。
...
新题会制造未来复习负担，因此调度器必须提供 `newItemAllowance`。MindTrain 只把获准引入的新题同步给 Anki，不能把整个领域题库一次性导入为 New Card。
...
- 可分享的训练内容、私人学习记录、Anki Collection 和数据库分别备份与授权。
...
产品阶段、完成标准和非目标以[产品路线图](产品路线图.md)为准；本节只描述对应的架构演进。
...
- 提供 Dashboard、Web 训练、实例设置、知识目录和管理概览。
- 支持多个用户自建训练领域、领域内调度隔离和目录树查询。
- 提供真实学习指标、薄弱与擅长内容以及 Scheduler 健康状态。

状态：已完成并部署。

### M4：可管理的个人训练闭环

- 补充题库查询、来源与版本治理、可复习题和错题明细。
- 提供近期 Session、单次训练详情和历史趋势 API。
- 让 Dashboard 指标可以下钻到对应知识点、题目和训练记录。

### M5：数据安全与可移植性

- 提供备份、恢复验证、升级前检查和实例诊断。
- 分别导入导出训练领域内容与个人学习记录。
- 导入过程支持预览、冲突报告、版本校验和明确确认。

### M6：内容质量与训练形式

- 增加来源覆盖、新鲜度、待复核内容和资料变更追踪。
- 建立生成题质量评测和 Prompt 回归。
- 在确定评分契约后扩展判断题、简答题或代码分析题。

### M7：FSRS 与 Anki 调度生态

- 稳定 Scheduler Provider 和 Review Event 契约。
- 验证 FSRS Provider，以及调度切换、迁移和对账。
- 实现可选 Anki Bridge、稳定 Card 映射、同步恢复和兼容矩阵。

### M8：多人协作

- 支持多用户身份、权限、审计和共享领域治理。
- 严格隔离共享训练内容与个人学习状态。
- 在实际需求明确后评估团队内容发布机制。
